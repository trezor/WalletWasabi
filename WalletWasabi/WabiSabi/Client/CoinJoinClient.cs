...
public class CoinJoinClient
{
	...
	private async Task<IEnumerable<TxOut>> ProceedWithOutputRegistrationPhaseAsync(uint256 roundId, ImmutableArray<AliceClient> registeredAliceClients, CancellationToken cancellationToken)
	{
		...
		AmountDecomposer amountDecomposer = new(roundParameters.MiningFeeRate, roundParameters.AllowedOutputAmounts, (int)availableVsize, isTaprootAllowed, SecureRandom);
		...
		var outputTxOuts = GetTxOuts(outputValues, DestinationProvider).ToArray();
