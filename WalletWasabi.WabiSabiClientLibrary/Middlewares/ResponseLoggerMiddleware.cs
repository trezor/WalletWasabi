...
public class ResponseLoggerMiddleware
{
	...
	private static async Task<string> GetResponseBody(HttpResponse httpResponse)
	{
	...
	private static async Task LogResponse(HttpResponse response)
	{
		Logger.LogInfo($"Response body: {await GetResponseBody(response)}");
	}
}
